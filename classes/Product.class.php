...
class Product extends Db
{
	...
	public function basic_seachByCategory($currPage = 1, $key = '', $cat_id = 'all')
	{
		...
		$sql = "SELECT Count(*)
					...
					WHERE 1";
		if ($cat_id != 'all')
			$sql .= " and category.cat_id = $cat_id";
		if ($key != '')
			$sql .= " and product.product_name like '%$key%'";
		//echo $sql;
	...
	public function searchForAdmin($tensp = '', $tugia = '', $dengia = '', $tungay = '', $denngay = '', $maloai = '', $mancc = '', $currPage = 1)
	{
		$arr_cons = array();
		if ($tensp != '')
			$arr_cons["`product`.product_name"] = "like '%" . $tensp . "%'";
		if ($tugia != '' && $dengia != '')
			$arr_cons["`product`.product_price"] = "BETWEEN " . $tugia . " AND " . $dengia;
		if ($tungay != '' && $denngay != '')
			$arr_cons["`product`.product_date"] = "BETWEEN '" . $tungay . "' AND '" . $denngay . "'";
		if ($maloai != '' && $maloai != 'all')
			$arr_cons["`category`.cat_id"] = "= " . $maloai;
		if ($mancc != '' && $mancc != 'all')
			$arr_cons["`provider`.provider_id"] = "= " . $mancc;
		// var_dump($arr_cons);
